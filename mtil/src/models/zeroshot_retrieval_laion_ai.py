...
def evaluate(model, dataloader, tokenizer, device, task_id=0, amp=True, recall_k_list=[5]):
    """
    ...
    for batch_images, batch_texts, inds in dataloader:
        ...
        with torch.no_grad(), autocast():
            batch_images_emb = F.normalize(model.encode_image(batch_images, task_id), dim=-1)
            batch_texts_emb = F.normalize(model.encode_text(batch_texts_tok, task_id), dim=-1)
